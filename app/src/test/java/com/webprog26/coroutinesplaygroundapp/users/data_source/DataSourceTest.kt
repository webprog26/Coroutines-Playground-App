...
import com.webprog26.coroutinesplaygroundapp.users.FakeLocalDataEmptyProvider
import com.webprog26.coroutinesplaygroundapp.users.FakeLocalDataSuccessProvider
import com.webprog26.coroutinesplaygroundapp.users.FakeLocalDataUpdater
import com.webprog26.coroutinesplaygroundapp.users.FakeNetworkDataProvider
import com.webprog26.coroutinesplaygroundapp.users.FakeNetworkEmptyDataProvider
import com.webprog26.coroutinesplaygroundapp.users.UiState
import com.webprog26.coroutinesplaygroundapp.users.mock.FakeErrorApi
import com.webprog26.coroutinesplaygroundapp.users.mock.FakeSuccessApi
...
class DataSourceTest {
    ...
    fun `verify that onUsersDataUpdated is called twice if both local and network data providers provide it with the data`() {
        runTest {
            ...
            Mockito.verify(onUsersDataUpdated, atLeast(2)).invoke(UiState.Success(
                FakeLocalDataSuccessProvider.USERS_LIST))
    ...
    fun `verify that onUsersDataUpdated is called only once if there is no local data and network data provider provides it with the data`() {
        runTest {
            ...
            Mockito.verify(onUsersDataUpdated, Mockito.times(1)).invoke(UiState.Success(
                FakeLocalDataSuccessProvider.USERS_LIST))
        }
    }
    ...
    fun `verify that onUsersDataUpdated is called with an Error if there was en error during loading data from remote server`() {
        runTest {
            ...
            val dataSource = createDataSource(
                FakeLocalDataEmptyProvider(), FakeNetworkDataProvider(
                    FakeErrorApi()
                )
            )
